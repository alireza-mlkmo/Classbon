...
import { IconChevronDown, IconChevronUp } from "@/app/_components/icons/indexp";
import { CourseLecture } from "@/types/course-lecture.interface";
import { CourseChapter } from "@/types/course-chapter.interface";
import { CourseCurriculumProps } from "./course-cuuriculum.types";
...
const CourseCurriculum: React.FC<CourseCurriculumProps> = ({ data }) => {
  ...
  return (
    <div className="accordion">
      {data?.map((chapter: CourseChapter, index: number) => (
        <div className="rounded-xl" key={`chapter-${chapter.id}`}>
          ...
          {activeIndex === index && (
            <div className={`accordion-content`}>
              ...
              >
                {chapter.lectures.map((lecture: CourseLecture) => (
                  <li
