...
const useVideo = (src: string) => {
  ...
  const seekToProgressClick = (e: React.MouseEvent<HTMLProgressElement>) => {
    ...
    if (videoRef.current && state.duration > 0) {
      videoRef.current.currentTime = clickedPercent * state.duration;
      dispatch({ type: "SET_PROGRESS", progress: clickedPercent * 100 });
    }
  };
